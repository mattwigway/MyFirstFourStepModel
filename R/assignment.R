...
calculate_direction_factors = function(nhts) {
    nhts$trips %>%
        mutate(
            ...
            persons_reported_trip = rowSums(across(starts_with("ONTD_P"), \(x) {
                x == 1
            }))
        ) %>%
        ...
        summarize(
            outbound = weighted.mean(
                WHYFROM %in% HOME_PURPOSES,
                WTTRDFIN / persons_reported_trip
            )
        ) %>%
        return()
}
...
calculate_occupancy_factors = function(nhts) {
    nhts$trips %>%
        ...
        group_by(trip_type, time_period) %>%
        summarize(
            average_occupancy = weighted.mean(persons_reported_trip, WTTRDFIN)
        ) %>%
        return()
}
...
apply_direction_factors = function(matrix, direction_factors) {
    ...
    inbound = mat_with_directions %>%
        mutate(across(c("Car", "Bike", "Walk", "Transit"), \(x) {
            x * (1 - outbound)
        }))

    # swap columns
    inbound[, c("orig_geoid", "dest_geoid")] = inbound[, c(
        "dest_geoid",
        "orig_geoid"
    )]
...
link_tracts = function(network, marginals) {
    all_nodes = tibble(
        lat = vertex_attr(network, "lat"),
        lon = vertex_attr(network, "lon")
    ) %>%
...
all_or_nothing = function(nodeflow_hourly, marginals, network, weights) {
    ...
    for (fr in unique(nodeflow_hourly$orig_node)) {
        ...
        incoming_edge = as.integer(get_edge_ids(
            network,
            as.vector(rbind(predecessors, 1:vcount(network)))
        ))
...
get_freeflow_weights = function(network) {
    return(
        edge_attr(network, "length_m") /
            1000 /
            edge_attr(network, "maxspeed_kph") *
            60
    )
}
...
frank_wolfe = function(
    ...
    nodeflow_hourly = odflow_hourly |>
        left_join(
            select(marginals$areas, orig_geoid = geoid, orig_node = node_idx),
            by = "orig_geoid"
        ) |>
        left_join(
            select(marginals$areas, dest_geoid = geoid, dest_node = node_idx),
            by = "dest_geoid"
        ) |>
    ...
    current_flows = all_or_nothing(
        nodeflow_hourly,
        marginals,
        network,
        get_freeflow_weights(network)
    )
    converged = F
    relgap = 0
    for (iteration in 1:maxiter) {
        ...
        aon_flows = all_or_nothing(
            nodeflow_hourly,
            marginals,
            network,
            get_congested_tt(network, current_flows)
        )
        ...
        if (relgap <= relgap_tol) {
            print(paste(
                "Assignment converged at iteration",
                iteration,
                "with relative gap",
                relgap
            ))
    ...
    if (!converged) {
        print(
            "Assignment did not converge, returning flows with relative gap",
            relgap
        )
    }
