...
map_trip_distribution = function(model, flows, timeperiod, triptype, origin_tract) {
  ...
  breaks = round(
    signif(wtd.quantile(plot_data$n_trips, plot_data$n_trips, c(0, 0.2, 0.4, 0.6, 0.8, 1)), digits = 1),
    digits = 0
  )
  names(breaks) = NULL
  ...
  plot_data %>%
    ...
    labs(
      fill = paste0(
        "Number of trips\ndestined for tract\n",
        "(total: ",
        format(round(sum(from_tract$n_trips)), scientific = F),
        ")"
      )
    ) +
...
NE_CITIES = read_sf(system.file(
  "extdata/naturalearth/ne_10m_populated_places_simple.shp",
...
label_cities = function(model, buffer = 500) {
    ...
    for (yquad in c(-1, 1)) {
      for (angle in c(0, 22.5, 45, 67.5)) {
        ...
        r = append(
          ...
          geom_sf_text(
            data = cities,
            aes(label = name, fill = NULL, linewidth = NULL),
            color = "white",
            position = position_nudge(xoff, yoff - 5000)
          )
        )
      }
    }
  ...
  r = append(
    ...
    geom_sf_text(
      ...
      aes(label = name, fill = NULL, linewidth = NULL),
      color = "black",
      position = position_nudge(0, -5000)
    )
  )
